...
void BLACKOUT()
{
	...
	{ //iterate through activated channels
		switchOFF(i);
	}
}
...
void ChannelTOGGLE(int chnum)
{ // Toggle Channel from OFF to ON (or ON to OFF)
	if (Channel[chnum][0] < 3)
	{ // Single Pin Channel, set pin to MAX or MIN
		if (Channel[chnum][4] < 30)
		{					 // PIN is mostly off, turn on
	...
	{ // RGB Channel, sum values and round to
...
void initChannelIO()
{ // iterate through Channel Array and setup all pins
